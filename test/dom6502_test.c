...
int main() {
	...
	// TEST NOK, but is the same as https://www.masswerk.at/6502/ 
	// end: decimal mode
	...
	a_brk();
	run_6502();
	assert_reg_equals(&sr, 0x30, "cmp [4]");
	...
	a_brk();
	run_6502();
	assert_reg_equals(&sr, 0xB0, "cmp [5]");
	...
	a_brk();
	run_6502();
	assert_reg_equals(&sr, 0xB0, "cmp [6]");
	...
	a_brk();
	run_6502();
	assert_reg_equals(&sr, 0x31, "cmp [7]");
	// end: cmp
